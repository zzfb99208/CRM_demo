...
| 角色 | 用户名 | 密码 | 可见模块 |
|------|--------|------|---------|
| 业务员 | sales | 123456 | 客户管理、产品总表、PO、审核历史&形式发票、PL |
| 审核者 | approver | 123456 | PI审核&审核历史、客户管理(只读)、产品总表(只读)、装箱单 |
...
**④ 驳回原因显示**
- 当 PI 被驳回时，业务员在 PI 详情页和审核历史中可以看到红色的驳回原因
- 审核者在已审核列表中点击已驳回的 PI，也能看到驳回原因
- 驳回原因存储在 `proforma_invoice.reject_reason` 字段

**⑤ 审核者界面合并**
- 审核者侧边栏：原"PI 审核"和"形式发票"合并为 **"PI审核&审核历史"**
- 进入后默认显示待审核列表，Tab 切换查看已审核历史
- 已审核历史支持按状态（通过/驳回）筛选

**⑥ 业务员审核历史**
- 业务员侧边栏：原"形式发票"改为 **"审核历史&形式发票"**
- 进入后默认为 PI 列表，增加"审核状态"列显示当前审核进度
- 被驳回的 PI 行显示红色驳回原因，点击可查看详情
...
│  Tab 已审核:                                                │
│  状态筛选: [全部 ▼] [通过] [驳回]                            │
│  Contract No       │ 业务员 │ 审核结果 │ 审核时间  │ 驳回原因  │
